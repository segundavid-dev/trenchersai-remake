...
import { MARKET_TICKER_DATA } from "@/lib/constants";

/* Sub components */

const OrderBook = ({ items }: { items: OrderBookItem[] }) => {
  const sellOrders = items.slice(0, 8);
  const buyOrders = items.slice(8, 16);
      ...
      <div className="flex flex-col gap-1.5">
        {sellOrders.map((item, i) => (
          <div key={`sell-${i}`} className="flex items-center gap-2">
            ...
            />
            <span className="text-[9px] text-red-500/60 ml-auto">{item.price}</span>
          </div>
        ))}
        ...
        {buyOrders.map((item, i) => (
          <div key={`buy-${i}`} className="flex items-center gap-2">
            <div 
              ...
              style={{ width: `${item.width}%` }}
            />
            <span className="text-[9px] text-green-500/60 ml-auto">{item.price}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  <div className="border border-white/[0.06] bg-white/[0.02] p-4 relative overflow-hidden">
    ...
    <div className="flex flex-col gap-3">
      <div className="flex justify-between items-center text-[9px]">
        <span className="text-white/30 uppercase">Mint Authority</span>
        <span className="text-green-500 font-bold">REVOKED</span>
      </div>
      <div className="flex justify-between items-center text-[9px]">
        <span className="text-white/30 uppercase">LP Status</span>
        <span className="text-green-500 font-bold">BURNED</span>
      </div>
      <div className="flex justify-between items-center text-[9px]">
        <span className="text-white/30 uppercase">Dev Wallet</span>
...
const MarketTicker = () => (
  <div className="absolute bottom-0 left-0 right-0 bg-brand/5 border-t border-brand/20 py-1.5 px-4 overflow-hidden">
    <div className="flex gap-8 animate-ticker">
      {[...Array(4)].map((_, i) => (
        <div key={i} className="flex gap-8">
          {MARKET_TICKER_DATA.map((ticker, idx) => (
            <span key={idx} className="text-[9px] text-white/60">
              {ticker.pair}
              <span className={ticker.isSignal ? "text-brand font-bold" : ticker.isPositive ? "text-green-500" : "text-red-500"}>
                {ticker.change}
              </span>
            </span>
          ))}
...
export function MarketHUD() {
  ...
  return (
    <div className="relative w-full h-full bg-[#050505] p-6 font-mono overflow-hidden">
      ...
      <div className="relative z-10 grid grid-cols-2 gap-6 h-full pb-8">
